...
// ══════════════════════════════════════════════════════════════
//  « Dossiers MyAfroOrigins » — reprise de dossiers bloqués.
//
//  Ce bloc vivait codé en dur dans /admin/documents : l'agent n'en voyait
//  rien, alors que c'est lui qui traite ces clients (Nadjath TIAMIOU en
//  premier lieu). Il est désormais monté À L'IDENTIQUE dans les deux panels,
//  sans copie : ce qui change ici change des deux côtés.
//
//  Le lien envoyé au client ouvre le formulaire du RÉCAP MyAfroOrigins
//  (/services/recap-myafroorigins/reprise), pas celui de la demande de
//  nationalité : ces clients ont déjà leur dossier chez MyAfroOrigins, on le
//  vérifie. Les demandes reçues arrivent dans la section « Récaps » en
//  dessous ; la liste « en attente de revue » ne sert plus qu'aux dossiers
//  déposés avant ce changement.
// ══════════════════════════════════════════════════════════════
import { useState, useEffect, useCallback } from 'react'
import { FileText, Envelope as Mail, Link as Link2, Copy, Check, CircleNotch as Loader2, Download, Eye, ArrowRight, X, Image as ImageIcon, ShieldCheck, Users, FolderSimplePlus as FolderInput, WarningCircle } from '@phosphor-icons/react'
...
interface FactureChoix { id: string; numero: string; client_nom: string | null; client_prenom: string | null; total: number; currency: string | null; status: string | null }

export default function MyafroRepriseSection() {
    // ── MyAfroOrigins ──
    ...
    const [invoices, setInvoices] = useState<FactureChoix[]>([])
    ...
    /* Seules les factures PAYÉES sont proposées : le serveur refuse les
       autres (« déjà payé » sans encaissement réel fausserait les comptes),
    ...
    const fetchInvoices = async () => {
        try {
            ...
            setInvoices((json.factures || []).filter((f: FactureChoix & { source_ref?: string | null }) => f.status === 'paye' && !f.source_ref))
        } catch (e) {
    ...
    const sendInvite = async () => {
        ...
        if (linkPaid && !selectedInvoiceId) { alert('Choisissez la facture qui prouve le règlement, ou décochez « déjà payé ».'); return }
    ...
    const generateLink = async () => {
        setLienErreur('')
        if (linkPaid && !selectedInvoiceId) {
            setLienErreur('Choisissez la facture qui prouve le règlement, ou décochez « déjà payé ».')
            return
        }
        setLienEnCours(true)
        try {
            ...
            if (!res.ok || !data.link) throw new Error(data.error || `Génération impossible (HTTP ${res.status}).`)
            setGenLink(data.link)
        } catch (e) {
            setLienErreur(e instanceof Error ? e.message : 'Génération impossible.')
        } finally { setLienEnCours(false) }
    }
                ...
                <section>
                    ...
                    <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 mb-6">
                        ...
                        <p className="text-[11px] text-gray-500 mb-4">
                            Si le client a déjà réglé ces frais de façon manuelle, cochez cette case et sélectionnez la facture associée. Le formulaire ne lui demandera aucun paiement, et la facture sera rattachée à sa demande.
                        </p>
                        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                            ...
                            {linkPaid && (
                                ...
                                >
                                    <option value="" className="bg-[#0d1424] text-gray-400">{invoices.length ? '-- Choisir une facture payée --' : '-- Aucune facture payée disponible --'}</option>
                                    {invoices.map(inv => (
                                        <option key={inv.id} value={inv.id} className="bg-[#0d1424] text-white">
                                            {inv.numero} : {`${inv.client_nom || ''} ${inv.client_prenom || ''}`.trim() || 'Sans nom'} ({Number(inv.total || 0).toLocaleString('fr-FR')} {inv.currency === 'EUR' ? '€' : inv.currency === 'USD' ? '$' : inv.currency === 'GBP' ? '£' : 'FCFA'})
                                        </option>
                                    ))}
                                </select>
                            )}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                        <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-5">
                            ...
                            <p className="text-[11px] text-gray-500 mb-4">Le client reçoit un lien personnel vers le formulaire de vérification MyAfroOrigins{linkPaid ? ' (frais déjà réglés)' : ' (frais de reprise : 50 €)'}.</p>
                    ...
                    {/* Liste des dossiers déposés AVANT le passage au formulaire récap :
                        les nouvelles demandes arrivent dans « Récaps » ci-dessous. */}
                    {(myafroLoading || myafroApps.length > 0) && (
                    ...
                    {!myafroLoading && myafroApps.length === 0 && (
                        <p className="text-[11px] text-gray-500 flex items-center gap-2">
                            ...
                            Les demandes envoyées depuis ces liens arrivent dans « Récaps MyAfroOrigins », juste en dessous.
                        </p>
                    )}
                </section>
