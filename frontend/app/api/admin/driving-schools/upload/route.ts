...
export async function POST(request: NextRequest) {
    ...
    if (files.length > 10) return NextResponse.json({ error: 'Maximum 10 images par envoi.' }, { status: 400 })
...
export async function DELETE(request: NextRequest) {
    ...
    if (idx === -1) return NextResponse.json({ success: true })
