...
import {
    Globe2, CheckCircle2, XCircle, Clock, Download,
    User, Mail, Search, ChevronDown, ChevronUp, MapPin,
    CreditCard, Plus, Trash2, Save, Eye
} from 'lucide-react'
...
interface Application {
    ...
    amount: number; currency: string; payment_status: string; payment_method: string
}

interface PaymentMethod { id: string; name: string; type: string; instructions: string; is_active: boolean; sort_order: number; logo_url: string }
...
export default function AdminNationalitePage() {
    const [tab, setTab] = useState<'demandes' | 'paiements'>('demandes')
    ...
    const [newMethod, setNewMethod] = useState({ name: '', type: 'mobile_money', instructions: '', logo_url: '' })
    ...
    useEffect(() => { fetchApps(); fetchMethods() }, [filter, search])
    ...
    const addPaymentMethod = async () => {
        ...
        await supabase.from('payment_methods').insert({ ...newMethod, is_active: true, sort_order: methods.length + 1 })
        setNewMethod({ name: '', type: 'mobile_money', instructions: '', logo_url: '' })
        ...
        <div className="min-h-screen bg-[#0a0f14] py-8 px-4">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex gap-2 mb-6">
                    {[{ k: 'demandes', l: 'Demandes' }, { k: 'paiements', l: 'Moyens de paiement' }].map(t => (
                        <button key={t.k} onClick={() => setTab(t.k as any)} className={`text-xs font-bold px-5 py-2.5 rounded-xl transition-all ${tab === t.k ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-white/5 text-gray-500 border border-white/5'}`}>
                            {t.k === 'paiements' && <CreditCard size={12} className="inline mr-1.5" />}{t.l}
                        </button>
                    ))}
                </div>
                ...
                {tab === 'paiements' ? (
                    <div className="space-y-4">
                        <p className="text-xs text-gray-500">Les moyens de paiement actifs seront affichés aux demandeurs à l'étape finale du formulaire.</p>
                        {methods.map(m => (
                            <div key={m.id} className="bg-white/[0.03] border border-white/5 rounded-xl p-5 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className={`w-3 h-3 rounded-full ${m.is_active ? 'bg-emerald-500' : 'bg-gray-600'}`} />
                                    <div>
                                        <p className="text-sm font-bold text-white">{m.name}</p>
                                        <p className="text-[10px] text-gray-500">{m.type} — {m.instructions || 'Pas d\'instructions'}</p>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={() => toggleMethod(m.id, m.is_active)} className={`text-[10px] font-bold px-3 py-1.5 rounded-lg ${m.is_active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-gray-500/20 text-gray-400'}`}>{m.is_active ? 'Actif' : 'Inactif'}</button>
                                    <button onClick={() => deleteMethod(m.id)} className="bg-red-500/10 text-red-400 p-1.5 rounded-lg hover:bg-red-500/20"><Trash2 size={14} /></button>
                                </div>
                            </div>
                        ))}
                        {addingMethod ? (
                            <div className="bg-white/[0.03] border border-emerald-500/20 rounded-xl p-5 space-y-3">
                                <div className="grid grid-cols-2 gap-3">
                                    <input value={newMethod.name} onChange={e => setNewMethod({ ...newMethod, name: e.target.value })} placeholder="Nom (ex: MTN Mobile Money)" className="bg-white/5 border border-white/10 rounded-xl py-2.5 px-4 text-white text-sm focus:outline-none" />
                                    <select value={newMethod.type} onChange={e => setNewMethod({ ...newMethod, type: e.target.value })} className="bg-white/5 border border-white/10 rounded-xl py-2.5 px-4 text-white text-sm focus:outline-none">
                                ...
                                <input value={newMethod.instructions} onChange={e => setNewMethod({ ...newMethod, instructions: e.target.value })} placeholder="Instructions (ex: Envoyer au +229 XX XX XX XX)" className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 px-4 text-white text-sm focus:outline-none" />
                                <input value={newMethod.logo_url} onChange={e => setNewMethod({ ...newMethod, logo_url: e.target.value })} placeholder="URL du logo (optionnel)" className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 px-4 text-white text-sm focus:outline-none" />
                                <div className="flex gap-2">
                                    <button onClick={addPaymentMethod} className="bg-emerald-500 text-white font-bold text-xs px-4 py-2 rounded-lg flex items-center gap-1"><Save size={12} /> Ajouter</button>
                                    <button onClick={() => setAddingMethod(false)} className="bg-white/5 text-gray-400 text-xs px-4 py-2 rounded-lg">Annuler</button>
                                </div>
                            </div>
                        ) : (
                            <button onClick={() => setAddingMethod(true)} className="bg-white/5 hover:bg-white/10 text-white font-bold text-xs px-4 py-3 rounded-xl transition-all flex items-center gap-2 w-full justify-center border border-dashed border-white/10"><Plus size={14} /> Ajouter un moyen de paiement</button>
                        )}
                    </div>
                ) : (
                    <>
                                        ...
                                        {isOpen && (
                                            <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} className="border-t border-white/5 p-5 space-y-4">
                                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
                                                    {[['Nationalité', a.nationalite], ['Profession', a.profession], ['Téléphone', a.telephone], ['Ancêtre', `${a.ancestor1_nom} (${a.ancestor1_lien_parente})`], ['Paiement', `${a.amount} ${a.currency} — ${a.payment_status}`], ['Méthode', a.payment_method]].map(([k, v], j) => v && (
